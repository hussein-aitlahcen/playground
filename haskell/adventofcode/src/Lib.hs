...
day8 :: Level
day8 content = do
  let (Right i) = mapM readInstruction . nonEmptyLines $ content
  print $ interpret M.empty i 0
  return (0, 0)
  where
    ...
    interpret !regs !stack !i
      | i >= length stack = maximum . M.elems $ regs
      | otherwise = let (Instruction ir im imv ic) = stack !! i
                    ...
                    in
                      if validCondition ic then
                        interpret (M.alter nextValue ir regs) stack nextIndex
                      else
                        interpret regs stack nextIndex
      where
